...
namespace FuDong.DAL.UnitOfWorks
{
    ...
    public abstract class UnitOfWorkContextBase : IUnitOfWorkContext
    {
        ...
        /// </summary>
        /// <returns></returns>
        public int Commit(bool isSave=false)
        {
            if (IsCommitted && isSave==false)
            {
                return 0;
            }
            try
            ...
            catch (DbUpdateException e)
            {
                if (e.InnerException != null && e.InnerException.InnerException is SqlException)
                {
                    ...
                    string msg = SQLErrorMessage.SqlMessage(sqlEx.Number);
                    throw new DataAccessException(string.Format(Resources.CommitDataError, msg), sqlEx); //"提交数据更新时发生异常："
                }
                throw;
            }
        }
