...
namespace GanGao.API
{
    ...
    public class AuthorizationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        {
            //Console.WriteLine("AuthorizationServerProvider");
            //var con = RegisgterMEF.regisgter();
            //if (userService == null)
            //{
            //    var obj = Startup.cfg.DependencyResolver;
            //    var con = obj as MefDependencySolver;

            //    ////var con = System.Web.Http.GlobalConfiguration.Configuration.DependencyResolver as MefDependencySolver;
                //con.Container.ComposeParts(this);
            //}
        ...
        public override Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            if (userService == null)
            {
                //context.SetError("invalid_userService", "userService is not valid");
                //return;
                var obj = Startup.cfg.DependencyResolver;
                var con = obj as MefDependencySolver;
                //var obj = System.Web.Http.GlobalConfiguration.Configuration.DependencyResolver;
                Console.WriteLine(obj.GetType());
                //con.ComposeParts

            }
            Console.WriteLine("UserName:{0} Password:{1} ClientId:{2}", context.UserName,context.Password,context.ClientId);
            ...
            var signResult = userService.ValidatorUserAsync(context.UserName, context.Password).Result;
            if (signResult.ResultType != OperationResultType.Success)
            {
                context.SetError("invalid_grant", signResult.Message);
                return Task.FromResult<object>(null);
            }
            //var user = await userService.FindByNameAsync(context.UserName);
            //if (user == null)
            //{
